...
def send_to_backend(detection_result, backend_url="http://localhost:8080/api/detection"):
    """Send detection result to Go backend for filtering, cooldown check, and clipping."""
    try:
        payload = detection_result.model_dump()
        response = requests.post(backend_url, json=payload, timeout=10)
        if response.status_code == 200:
            result = response.json()
            alerts_count = result.get("alerts_generated", 0)
            if alerts_count > 0:
                print(f"  -> Backend: {alerts_count} alert(s) generated")
                for alert in result.get("alerts", []):
                    print(f"     [{alert.get('priority', '?').upper()}] {alert.get('policy_name', '?')} clip={alert.get('clip_file', 'none')}")
            else:
                print(f"  -> Backend: no alert (filtered/cooldown)")
        else:
            print(f"  -> Backend error: HTTP {response.status_code}")
    except requests.ConnectionError:
        print(f"  -> Backend offline (tidak bisa connect ke {backend_url})")
...
def main():
    ...
    backend_url = config.get("backend_url", "http://localhost:8080/api/detection")
    ...
    try:
        while True:
            for cam_data in streamers:
                ...
                if len(violations) > 0:
                    timestamp = round(streamer.get_video_timestamp_seconds(), 2)
                    ...
                    # Send to Go backend for filtering + cooldown + clipping
                    send_to_backend(result, backend_url)
